...
def init_bar(total: int):
    global total_progress
    total_progress = total


def reset():
    global current_step, percentage, current_length
    current_length = 0
    current_step = 0
    percentage = 0
    render()
...
def render():
    sys.stdout.write("{0:}{1:}{2:} {3:.2f}%\r".format(style_edges[0], style_fill * current_length + ' ' * (width - current_length), style_edges[1], float(percentage * 100)))
    sys.stdout.flush()


def step(progress: float):
    global current_step, total_progress, percentage, current_length
    current_step += progress
    # print(str(progress) + ", " + str(self.current_step))
    if current_step > total_progress: current_step = total_progress
    percentage = current_step / total_progress
    current_length = int(percentage * width)
    render()
